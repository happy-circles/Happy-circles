...
import { counterFinancialRequest, createFinancialRequest } from './financial-request';
...
describe('FinancialRequest', () => {
  ...
  it('closes the original request and creates a new counteroffer request', () => {
    ...
    const counter = createFinancialRequest({
      ...
      creditorUserId: USER_B,
      amountMinor: 2000,
      description: 'Counter lunch',
      parentRequestId: original.id,
    });

    const result = counterFinancialRequest(original, counter);

    expect(result.closedRequest.status).toBe('countered');
    expect(result.counterRequest.parentRequestId).toBe(original.id);
  });
});
