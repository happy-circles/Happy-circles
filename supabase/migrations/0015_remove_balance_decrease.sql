...
begin
  create temporary table tmp_deprecated_request_ids (
    id uuid primary key
  ) on commit drop;

  create temporary table tmp_deprecated_transaction_ids (
    id uuid primary key
  ) on commit drop;

  insert into tmp_deprecated_request_ids (id)
  with recursive deprecated_requests as (
    select fr.id
    from public.financial_requests fr
    where fr.request_type::text = 'balance_decrease'
    union all
    select child.id
    from public.financial_requests child
    join deprecated_requests parent on child.parent_request_id = parent.id
  )
  select distinct id
  from deprecated_requests;

  insert into tmp_deprecated_request_ids (id)
  with recursive deprecated_reversals as (
    select reversal.id
    from public.financial_requests reversal
    join public.ledger_transactions target_tx on target_tx.id = reversal.target_ledger_transaction_id
    where target_tx.origin_request_id in (select id from tmp_deprecated_request_ids)
    union all
    select child.id
    from public.financial_requests child
    join deprecated_reversals parent on child.parent_request_id = parent.id
  )
  select distinct id
  from deprecated_reversals
  on conflict (id) do nothing;

  insert into tmp_deprecated_transaction_ids (id)
  select lt.id
  from public.ledger_transactions lt
  where lt.transaction_type::text = 'balance_decrease_acceptance'
     or lt.origin_request_id in (select id from tmp_deprecated_request_ids)
  on conflict (id) do nothing;

  delete from public.audit_events
  where request_id in (select id from tmp_deprecated_request_ids)
     or (entity_type = 'financial_request' and entity_id in (select id from tmp_deprecated_request_ids))
     or (entity_type = 'ledger_transaction' and entity_id in (select id from tmp_deprecated_transaction_ids));

  delete from public.pair_net_edges_cache;

  delete from public.ledger_entries
  where ledger_transaction_id in (select id from tmp_deprecated_transaction_ids);

  delete from public.ledger_transactions
  where id in (select id from tmp_deprecated_transaction_ids);

  delete from public.financial_requests
  where id in (select id from tmp_deprecated_request_ids);
...
drop view if exists public.v_inbox_items;
drop view if exists public.v_relationship_history;

alter type public.request_type rename to request_type_legacy;
create type public.request_type as enum ('balance_increase', 'transaction_reversal');

alter table public.financial_requests
  alter column request_type type public.request_type
  using request_type::text::public.request_type;

alter type public.ledger_transaction_type rename to ledger_transaction_type_legacy;
...
alter table public.ledger_transactions
  alter column transaction_type type public.ledger_transaction_type
  using transaction_type::text::public.ledger_transaction_type;

drop function if exists public.create_balance_request(
  uuid,
  text,
  public.request_type_legacy,
  uuid,
  uuid,
  uuid,
  bigint,
...
alter view public.v_inbox_items set (security_invoker = true);

drop type public.request_type_legacy;
drop type public.ledger_transaction_type_legacy;
